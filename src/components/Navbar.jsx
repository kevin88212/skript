...
import { Sword, Utensils, Calendar, Dumbbell, Shield, User } from 'lucide-react';
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 md:static md:flex md:flex-col md:w-64 md:min-h-screen">
      ...
      <div className="hidden md:flex flex-col h-screen sticky top-0 card-dark border-r border-indigo-500/20 p-4">
          ...
          {links.map(({ to, icon: Icon, label }) => (
            <NavLink
              ...
              className={({ isActive }) =>
                ...
                ${isActive
                  ? 'bg-indigo-500/20 text-indigo-300 border border-indigo-500/40 glow-indigo'
        ...
        <div className="mt-auto text-center text-xs text-gray-600 py-2">
          v1.0 · FitQuest RPG
        </div>
      </div>

      {/* Mobile bottom bar */}
      <div className="md:hidden flex bg-gray-950/95 backdrop-blur-xl border-t border-indigo-500/20 px-2 py-2">
        {links.map(({ to, icon: Icon, label }) => (
          <NavLink
            key={to}
            to={to}
