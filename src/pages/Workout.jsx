...
import { Dumbbell, Play, Check, ChevronDown, ChevronUp, Zap } from 'lucide-react';
import { useApp } from '../context/AppContext';
import { exercises, WORKOUT_PLANS } from '../data/exercises';

function ExerciseCard({ ex, onComplete, isCompleted }) {
            ...
            <div className="px-4 pb-4 space-y-3">
              ...
              <div className="space-y-2">
                {ex.steps.map((step, i) => (
                  <div key={i} className="flex gap-3">
                    <span className="w-5 h-5 rounded-full bg-indigo-500/20 text-indigo-300 text-xs flex items-center justify-center shrink-0 mt-0.5">
                      {i + 1}
                    </span>
              ...
              {!isCompleted && (
                <motion.button
                  ...
                  className="w-full py-3 rounded-xl bg-indigo-500 text-white font-semibold text-sm flex items-center justify-center gap-2 glow-indigo"
                >
                  <Check size={16} /> Übung abgeschlossen +{ex.xp} XP
                </motion.button>
              )}
            </div>
          </motion.div>
          ...
          <button
            ...
            className={`flex-1 py-2 px-3 rounded-xl text-xs font-semibold transition-all
              ${activeDay === i
                ...
                : 'card-dark text-gray-400 hover:text-gray-200 border border-gray-700/50'
              }`}
          >
            {d.day.split('–')[0].trim()}
          </button>
      ...
      <div className="card-dark rounded-2xl p-4 border border-indigo-500/25">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-xs text-neon-amber">
            {progress}/{dayExercises.length} Übungen
          </span>
        </div>
        <div className="w-full h-2 bg-gray-800 rounded-full overflow-hidden">
          <motion.div
            animate={{ width: `${(progress / dayExercises.length) * 100}%` }}
      ...
      <AnimatePresence>
        {allDone && !completedWorkoutToday && (
          <motion.div
            ...
            className="sticky bottom-24 md:bottom-4"
          >
            <button
